...
int main(int argc, char *argv[])
{
    ...
    char *task_description = "Solves the bachelorette dating problem, which is "
                             "an instance\nof the college admission problem, "
                             "for the data given in PREFERENCES_FILE";
    ...
    /* Solve the bachelorette dating problem */
...
void process_datafile(const char *data_fname, participantData_t *data)
{
    ...
    while (lines_read < data->num_people)
    {
        ...
        if (gender == MALE)
        {
            /* Men can only have one match */
            set_matches_data(data->people[people_index], 1);
        }

        lines_read++;
    }

    /* Read number of matches for each woman from file */
    lines_read = 0;
    while (lines_read < data->num_females)
    {
        int id;
        int max_matches;
        int people_index;
        ...
        people_index = data->num_males + id;
        set_matches_data(data->people[people_index], max_matches);
...
void print_data(participantData_t *data)
{
    ...
    for (i = 0; i < data->num_people; i++)
    {
        ...
        fprintf(stderr, "], max_matches: %d\n", this_person->max_matches);
    }
    fprintf(stderr, "\n");
}
...
void match_participants(participantData_t *data, struct cmdline_opt *options)
{
    ...
    while (!stable)
    {
        ...
        /* Loop through each male attempting to match if not already */
        for (i = 0; i < data->num_males; i++)
        {
            ...
            if (!(*curr_man->matches))
            {
                ...
                person_t *potential_match = get_next_candidate(curr_man, data);
                 ...
                 * already or if 'curr_man' is a better match for
                 * 'potential_match' than their current match
                 */
                ask_out(curr_man, potential_match, verbose);
            }
        }
    }
...
void print_matches(participantData_t *data, struct cmdline_opt *options)
{
    ...
    for (i = 0; i < data->num_males; i++)
    {
        ...
        if (*curr_man->matches)
        {
            printf("%d %d\n", curr_man->id, (*curr_man->matches)->id);
        }
        else if (verbose)
        {
